...
fn test_storage_bucket_crud() {
    let dir = tempdir();
    let engine = vexobj_storage::StorageEngine::new(dir.clone(), 1024 * 1024, true).unwrap();
...
async fn test_storage_object_crud() {
    ...
    // List objects
    let list = engine
        .list_objects(&vexobj_storage::ListObjectsRequest {
...
async fn test_object_too_large() {
    let dir = tempdir();
    let engine = vexobj_storage::StorageEngine::new(dir.clone(), 100, true).unwrap();
...
fn test_auth_keys() {
    let dir = tempdir();
    let auth = vexobj_auth::AuthManager::open(&dir.join("auth.db")).unwrap();

    // Create key
    let (key, raw) = auth
        .create_key(
            "test",
            vexobj_auth::Permissions {
                read: true,
                write: false,
                delete: false,
                admin: false,
            },
            vexobj_auth::BucketAccess::All,
        )
        .unwrap();
...
fn test_bucket_access_control() {
    ...
    let (key, _) = auth
        .create_key(
            "limited",
            vexobj_auth::Permissions::default(),
            vexobj_auth::BucketAccess::Specific {
...
fn test_presigned_urls() {
    let gen = vexobj_auth::PresignedUrlGenerator::new(b"test-secret-key");

    let req = vexobj_auth::PresignRequest {
...
#[tokio::test]
async fn test_cache() {
    let cache = vexobj_cache::Cache::new(1024 * 1024, None, 0);
...
async fn test_garbage_collection() {
    let dir = tempdir();
    let engine = vexobj_storage::StorageEngine::new(dir.clone(), 1024 * 1024, false).unwrap();
    ...
    let gc = vexobj_storage::GarbageCollector::new(dir);
...
async fn test_backup_and_restore() {
    ...
    let bm = vexobj_storage::BackupManager::new(dir.clone());
    ...
    let bm2 = vexobj_storage::BackupManager::new(restore_dir.clone());
...
async fn test_streaming_upload() {
    let dir = tempdir();
    let engine = vexobj_storage::StorageEngine::new(dir.clone(), 1024 * 1024 * 100, true).unwrap();

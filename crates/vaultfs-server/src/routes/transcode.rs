...
async fn submit_job(
    ...
    // for nothing.
    if vaultfs_processing::transcode_profile(&body.profile).is_none() {
        return (
            StatusCode::BAD_REQUEST,
            Json(json!({
                "error": format!("unknown profile '{}'", body.profile),
                "available": vaultfs_processing::TRANSCODE_PROFILES
...
async fn list_profiles() -> impl IntoResponse {
    let items: Vec<_> = vaultfs_processing::TRANSCODE_PROFILES
        .iter()
        .map(|p| {
            json!({
                "name": p.name,
